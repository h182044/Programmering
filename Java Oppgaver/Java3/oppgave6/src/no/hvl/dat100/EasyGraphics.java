...
/**
 * <p>
 * Klassen EasyGraphics tilbyr metoder for å tegne og flytte ("animere")
 * linjer, rektangel, sirkler, ellipser, sirkelbuer og tekst.
 * </p>
 *
 * <p>
 * Klassen er et lite overbygg på Swing. Ved å bruke EasyGraphics kan man bli
 * kjent med grafikk og animasjon på en enkel måte.
 * </p>
 *
 * <p>
 * Her er et eksempelprogram som gjør følgende:
 * </p>
 *
 * <ul>
 * <li>Setter opp grafikkvinduet med en tegneflate med tittel "GrafikkDemo" og
 * bredde=høyde=500 (makeWindow).</li>
 ...
 * 
 * <p>
 * For to-dimensjonale geometriobjekt kan man velge å tegne omriss
 * (drawRectangle) eller å fylle med farge (fillRectangle). Aktiv farge settes
 * med en egen metode setColor. Fonten til tekststrenger kan settes med metoden
 * setFont.
 * </p>
 *
 * <p>
 * Metoder som tegner objektene (f.eks. drawCircle og fillCircle) returnerer en
 * unik node-id, som senere kan brukes for å flytte eller reskalere objektet
 * (tekstobjekt kan ikke reskaleres). Alle objekt kan skjules/vises
 * (setVisible).
 * </p>
 *
 * <p>
 * Geometriobjekt posisjoneres, flyttes og reskaleres i forhold til et bestemt
 * punkt på objektet:
 * </p>
 *
 * <ul>
 ...
 *
 * <p>
 * Farten på en animasjon blir bestemt ved metoden setSpeed, der parameter 1 =
 * langsom, ..., 10 = hurtig. Det er også mulig å legge inn pauser i en
 * animasjon (metode pause).
 * </p>
 *
 * <p>
 * Nettsidene til
 * <a href="http://www.nettressurser.no/programmering" target="_top"> Forstå
 * programmering</a> forklarer hvordan man kan kompilere og kjøre
 * EasyGraphics-program.
 * </p>
 *
 * @version 1.1
 *
 *          <p>
 *          Sist endret: 19.03.2013
 *          </p>
 *
 */
public abstract class EasyGraphics extends JFrame {
	...
	/**
	 * Utfører tegne- og animasjons-operasjoner. Denne metoden må deklareres
	 * (overstyres). I grafikkprogram spiller denne rollen som "sjefsmetode" (a la
	 * main).
	 *
	 */
	...
	/**
	 * Tegner en sirkelbue med aktiv farge. Returnerer en unik id som kan brukes for
	 * å flytte sirkelbuen.
	 *
	 ...
	 *
	 */
	public int drawArc(int centerX, int centerY, int radiusX, int radiusY, int startAngle, int angleLength) {
		return makeArc(centerX, centerY, radiusX, radiusY, startAngle, angleLength, false);
	}

	/**
	 * Tegner en sirkel med aktiv farge. Returnerer en unik id som kan brukes for å
	 * flytte sirkelen.
	 *
	 * @param centerX x-koordinat til midtpunktet
	 * @param centerY y-koordinat til midtpunktet
	...
	/**
	 * Tegner en ellipse med aktiv farge. Returnerer en unik id som kan brukes for
	 * å flytte ellipsen.
	 *
	 * @param centerX x-koordinat til midtpunktet
	 * @param centerY y-koordinat til midtpunktet
	...
	/**
	 * Tegner en rett linje med aktiv farge. Returnerer en unik id som kan brukes
	 * for å flytte linjen.
	 *
	...
	/**
	 * Tegner et rektangel med aktiv farge. Returnerer en unik id som kan brukes for
	 * å flytte rektangelet.
	 *
	...
	/**
	 * Tegner tekst med aktiv font og farge. Returnerer en unik id som kan brukes
	 * for å flytte teksten.
	 *
	...
	/**
	 * Fyller en sirkelbue ("kakestykke") med aktiv farge. Returnerer en unik id som
	 * kan brukes for å flytte sirkelbuen.
	 *
	 * @param centerX     x-koordinat til midtpunktet
	 * @param centerY     y-koordinat til midtpunktet
	 * @param radiusX     "horisontal radius", må være større enn 0
	 ...
	 *
	 */
	public int fillArc(int centerX, int centerY, int radiusX, int radiusY, int startAngle, int angleLength) {
		return makeArc(centerX, centerY, radiusX, radiusY, startAngle, angleLength, true);
	}

	/**
	 * Fyller en sirkel med aktiv farge. Returnerer en unik id som kan brukes for å
	 * flytte sirkelen.
	 *
	 * @param centerX x-koordinat til midtpunktet
	 * @param centerY y-koordinat til midtpunktet
	 ...
	 *
	 */
	public int fillEllipse(int centerX, int centerY, int radiusX, int radiusY) {
		return makeEllipse(centerX, centerY, radiusX, radiusY, true);
	}

	/**
	 * Fyller et rektangel med aktiv farge. Returnerer en unik id som kan brukes for
	 * å flytte rektangelet.
	 *
	 * @param minX   x-koordinat for venstre side
	 * @param minY   y-koordinat for øvre side
	...
	/**
	 * Leser inn en tekststreng fra brukeren, via et tekstfelt øverst i vinduet.
	 *
	...
	/**
	 * Flytter ("animerer") figuren med gitt node-id til et gitt punkt langs en rett
	 * linje. Se tilsvarende moveXXX-metoder tilpasset de ulike figurtypene for hvor
	 * på figuren punktet (x,y) refererer.
	 *
	 * @param id node-id
	...
	/**
	 * Flytter ("animerer") sirkelbuen med gitt node-id til et gitt punkt langs en
	 * rett linje. Den nye posisjonen vil være midtpunktet i "tilhørende" sirkel.
	 *
	 * @param id node-id
	 * @param x  x-koordinatet til ny posisjon
	 * @param y  y-koordinatet til ny posisjon
	...
	/**
	 * Flytter ("animerer") sirkelen med gitt node-id til et gitt punkt langs en
	 * rett linje. Den nye posisjonen vil være midtpunktet i sirkelen.
	 *
	 * @param id node-id
	 * @param x  x-koordinatet til ny posisjon
	 * @param y  y-koordinatet til ny posisjon
	...
	/**
	 * Flytter ("animerer") ellipsen med gitt node-id til et gitt punkt langs en
	 * rett linje. Den nye posisjonen vil være midtpunktet i ellipsen.
	 *
	 * @param id node-id
	 * @param x  x-koordinatet til ny posisjon
	 * @param y  y-koordinatet til ny posisjon
	...
	/**
	 * Flytter ("animerer") linjen med gitt node-id til et gitt punkt langs en rett
	 * linje. Den nye posisjonen vil være startpunktet til linjen.
	 *
	 * @param id node-id
	...
	/**
	 * Flytter ("animerer") rektangelet med gitt node-id til et gitt punkt langs en
	 * rett linje. Den nye posisjonen vil være øverste, venstre hjørne i
	 * rektangelet.
	 *
	 * @param id node-id
	 * @param x  x-koordinatet til ny posisjon
	...
	/**
	 * Flytter ("animerer") teksten med gitt node-id til et gitt punkt langs en rett
	 * linje. Den nye posisjonen vil være nedre, venstre punkt i teksten.
	 *
	 * @param id node-id
	 * @param x  x-koordinatet til ny posisjon
	 * @param y  y-koordinatet til ny posisjon
	...
	/**
	 * Skriver ut en tekst til konsollet på riktig tidspunkt (i forhold til
	 * pågående animasjoner).
	 *
	 * @param msg meldingen
	 *
	 */
	...
	/**
	 * Skriver ut en tekst med linjeskift til konsollet på riktig tidspunkt (i
	 * forhold til pågående animasjoner).
	 *
	 * @param msg meldingen
	 *
	 */
	...
	/**
	 * Endrer størrelse ("animerer") på linjen med gitt node-id. Det gjøres ved
	 * å forskyve endepunktet, startpunktet ligger fast.
	 *
	 * @param id node-id
	...
	/**
	 * Setter aktiv farge med et RGB-trippel. Geometriobjekt som blir tegnet vil
	 * heretter få denne fargen (helt fram til neste kall på setColor).
	 *
	 * @param r rød verdi mellom 0 og 255
	 * @param g grønn verdi mellom 0 og 255
	 * @param b blå verdi mellom 0 og 255
	...
	public void setColor(int r, int g, int b) {
		checkLaunched();
		boolean ok = checkInterval(r, 0, 255, "r") && checkInterval(g, 0, 255, "g") && checkInterval(b, 0, 255, "b");
		if (ok)
			color = new Color(r, g, b);
	}

	/**
	 * Setter aktiv font. Tekststrenger som blir tegnet vil heretter få denne
	 * fargen (helt fram til neste kall på setFont).
	 *
	...
	/**
	 * <p>
	 * Setter farten på animasjonen.
	 * </p>
	 *
	 * <p>
	 * Metoden tilbyr en meget enkel måte å styre farten til animasjoner på. Den
	 * setter kun inn en kort pause mellom hvert "steg".
	 * </p>
	 * <p>
	 * Hastigheten til en animasjon vil bla. variere med hvilken maskin man bruker,
	 * og om man f.eks. gjør (få og) lange eller (mange og) korte
	 * forflytninger/reskaleringer. I det siste tilfellet bør endringen være 4
	 * eller kortere i både x- og y-retning.
	 * </p>
	 * <p>
	 * For å oppnå ønsket fart må man eksperimentere litt i hvert enkelt
	 * tilfelle.
	 * </p>
	 * 
	...
	/*
	 * Legger inn-køen inn til i bufferet av køer for behandling på
	 * event-tråden.
	 */
	...
	/*
	 * Sjekker at det for en gitt id er opprettet en figur av korrekt type.
	 */
	...
	/*
	 * Sjekker at en parameter til en av EasyGraphics- metodene er innenfor et gitt
	 * intervall. Se også checkPositive over.
	 */
	...
	/*
	 * Sjekker at brukerprogrammet er startet opp med kall på launch (fra main).
	 */
	...
	/*
	 * Sjekker at en parameter til en av EasyGraphics- metodene er positiv. For å
	 * levere eventuell feilmelding på riktig tidspunkt (og deretter avbryte
	 * programkjøring), gjøres dette i et animasjonssteg.
	 */
	...
	/*
	 * Oppretter en instans av applikasjonen. Navnet på applikasjonen (programmet
	 * som brukeren skriver) avleses fra kallstakken til den aktive tråden. Det
	 * blir sjekket at den kjørbare klassen (den som deklarerer main) er en
	 * subklasse av EasyGraphics. Ved hjelp av ClassLoader-klassen blir det
	 * opprettet et objekt av den kjørbare klassen (fra navnet på klassen).
	 */
	 ...
	 */
	private int makeArc(int centerX, int centerY, int radiusX, int radiusY, int startAngle, int angleLength,
			boolean fill) {
		checkLaunched();
	 ...
	 */
	private int makeCircle(int centerX, int centerY, int radius, boolean fill) {
		checkLaunched();
	 ...
	 */
	private int makeEllipse(int centerX, int centerY, int radiusX, int radiusY, boolean fill) {
		checkLaunched();
		checkPositive(radiusX, "radiusX");
		checkPositive(radiusY, "radiusY");
	 ...
	 */
	private int makeLine(int startX, int startY, int endX, int endY, boolean fill) {
		checkLaunched();
	 ...
	 */
	private int makeRectangle(int minX, int minY, int width, int height, boolean fill) {
		checkLaunched();
		checkPositive(width, "width");
		checkPositive(height, "height");
...
/*
 * Et objekt av denne klassen representerer en sirkelbue (uten fyll), eller et
 * "kakestykke" (med fyll).
 */
class EGArc extends EGFigure {
	...
	public EGArc(int x, int y, int radiusX, int radiusY, int startAngle, int angleLength) {
		super(x, y);
	...
	public void draw(Graphics g) {
		if (visible) {
			super.draw(g);
			if (fill)
				g.fillArc(x - radiusX, y - radiusY, 2 * radiusX, 2 * radiusY, startAngle, angleLength);
			else
				g.drawArc(x - radiusX, y - radiusY, 2 * radiusX, 2 * radiusY, startAngle, angleLength);
		}
	}
	...
	public String toString() {
		return "[EGArc x=" + x + ", y=" + y + ", radiusX=" + radiusX + ", radiusY=" + radiusY + ", startAngle= "
				+ startAngle + ", angleLength" + angleLength + "]";
	}
...
/*
 * Klassen håndterer operasjonene (steg) som skal bli utført, typisk
 * tegneoperasjoner. Datastrukturen er logisk sett en kø, der hoved-tråden
 * setter steg-objekt inn bakerst og event-tråden henter ut først i køen. For
 * å redusere synkronisering er datastrukturen egentlig en kø av køer, med
 * synkroniserte metoder for å legge inn og ta ut en enkelt kø.
 *
 * Hvis det ikke er plass til å sette inn en ny kø, vil hoved-tråden vente.
 * Hvis det ikke er noen kø å hente vil event-tråden bare ignorere det (se
 * EGCanvas).
 */
class EGBuffer {
...
/*
 * Klassen representerer tegneflaten. Den har en liste med figur-objekt og får
 * en kø av operasjoner (steg) for utførelse. Brukerprogrammet, som kjører
 * på en egen tråd, legger nye steg (f.eks. en move-operasjon) inn på slutten
 * av køen, mens tegneflaten, som kjører som del av event-løkka, tar ut steg
 * fra begynnelsen av den samme køen.
 *
 * Tegne-operasjonene kontrolleres av en timer (javax.swing.Timer). Den kaller
 * jevnlig opp en hendelsesmetode (actionPerformed) med noen millisekunders
 * pause. Hver gang blir ett eller flere steg behandlet (noen steg blir kun
 * delvis behandlet), og oppdatert figurliste blir tegnet på nytt.
 */
class EGCanvas extends JPanel implements ActionListener {
	...
	/*
	 * Hent steg fra køen for utførelse. For å effektivisere animasjoner kan
	 * flere steg bli utført i ett kall. Dette vil f.eks. gjelde flere, korte
	 * flyttinger av forskjellige figurer. Motsatt, blir lengre flyttinger og
	 * større reskaleringer behandlet over flere kall.
	 */
	 ...
	 *
	 * TODO Følgende "avhengighet" bør fjernes: Figurene får først en (unik) id
	 * i hoved-tråden (se EasyGraphics), og blir tildelt id igjen ved innsetting
	 * nå, gitt ved posisjonen i figurlisten. Brukerprogrammet kan referere til
	 * slik id'er, og det er avgjørende at figurene blir opprettet i samme
	 * rekkefølge de to stedene i koden.
	 *
	 * Mulig løsning: Utvid figur-objekt med id første gang de opprettes, og la
	 * dette definere posisjonen i listen.
	 */
	...
	/*
	 * Objektvariablene (minX, minY, maxX, maxY) tar vare på den delen av
	 * tegneflaten som er endret siden forrige tegneoperasjon (paintComponent).
	 * Dette brukes for å effektivisere tegning (kun den delen som er endret blir
...
class EGCircle extends EGFigure {
	...
	public String toString() {
		return "[EGCircle x=" + x + ", y=" + y + ", radius=" + radius + "]";
	}
...
/*
 * Klassen inneholder diverse konstanter, referanse til køen av animasjonssteg,
...
class EGEllipse extends EGFigure {
	...
	public String toString() {
		return "[EGEllipse x=" + x + ", y=" + y + ", radiusX=" + radiusX + ", radiusY=" + radiusY + "]";
	}
...
/*
 * Klassen brukes for å avbryte programkjøringen. Hvis brukerprogrammet kaller
 * EasyGraphics-metoder med ulovlige parametre skal programkjøring avbrytes.
 * For at dette skal skje på korrekt tidspunkt i forhold til en animasjon, blir
 * operasjonen lagt inn som et steg i køen på vanlig måte.
 */
...
abstract class EGFigure {
	...
	/*
	 * For å effektivisere tegning under animasjon, blir kun den delen av vinduet
	 * som er oppdatert tegnet på nytt. Metodene minX, minY, maxX og maxY leverer
	 * det omsluttende rektangelet til hvert geometriobjekt, og overlap sjekker om
	 * dette rektangelet overlapper med den delen av vinduet som er oppdatert.
	 */
	...
	public boolean resize() {
		...
		if (Math.abs(width1 - getWidth()) < DELTA && Math.abs(height1 - getHeight()) < DELTA) {
			setWidth(width1);
			setHeight(height1);
			finished = true;
		}
...
/*
 * Klassen brukes for å avslutte en animasjon. Det innebærer å "vekke"
 * hoved-tråden.
 */
...
/*
 * Klassen brukes for å styre enten fart eller bakgrunnsfarge, og vil gjelde
 * for alle påfølgende operasjoner.
 */
...
/*
 * Klassen oppretter brukergrensesnittet på event-tråden, og håndterer
 * inndata fra bruker.
 */
class EGGui implements Runnable, ActionListener {
	...
	/*
	 * Oppretter brukergrensesnittet og gir beskjed om at setningene i
	 * brukerprogrammet kan startes opp.
	 */
	public void run() {
		makeGUI();
...
/*
 * Klassen brukes for å synkronisere koden i brukerprogrammet med
 * event-tråden. EasyGraphics tilbyr brukeren en "sekvensiell" måte å
 * programmere grafikk på, uten å måtte tenke på hendelser.
 *
 * For å få til dette må vi sørge for at brukerprogrammet (run) av og til
 * venter på at animasjoner som kjøres som på event-tråden blir fullført,
 * eller at innputt fra brukeren (getText i EasyGraphics) er lest inn.
 * Event-tråden vekker da brukerprogrammet opp igjen. Denne synkroniseringen
 * gjøres ved hjelp av klassen CountDownLatch.
 */
class EGLatch {
	...
	/*
	 * Metoden sørger for å sette hoved-tråden "på vent" til brukergrensesnittet
	 * er ferdig opprettet.
	 */
	...
	/*
	 * Metoden sørger for å vekke hoved-tråden når brukergrensesnittet er ferdig
	 * opprettet.
	 */
	...
	/*
	 * Metoden sørger for å sette hoved-tråden "på vent" til pågående
	 * animasjoner i event-tråden er ferdig behandlet.
	 */
	...
	/*
	 * Metoden sørger for å vekke hoved-tråden når vi er ferdige med å behandle
	 * animasjoner.
	 */
	...
	/*
	 * Metoden sørger for å sette hoved-tråden "på vent" til innlesing fra
	 * bruker er ferdig.
	 */
	...
	/*
	 * Metoden sørger for å vekke hoved-tråden når vi har lest inndata fra
	 * brukeren.
	 */
...
class EGLine extends EGFigure {
	...
	public String toString() {
		return "[EGLine x=" + x + ", y=" + y + ", width=" + width + ", height=" + height + "]";
	}
...
/*
 * Klassen brukes til å opprette et nytt geometriobjekt, dvs. tegne det for
 * første gang. Objektet legges til i listen (se EGCanvas) i konstruktøren,
 * men blir først synlig når steget blir utført (execute).
 */
...
/*
 * Klassen representerer forflytning av et figur-objekt langs en rett linje.
 * Endringen blir brutt ned i mange små steg, slik at man oppnår en
 * animasjonseffekt.
 */
...
/*
 * Klassen brukes for å gjøre utskrift til konsollet underveis i en animasjon,
 * slik at effekten kommer på forventet tidspunkt.
 */
...
class EGRectangle extends EGFigure {
	...
	public String toString() {
		return "[EGRectangle x=" + x + ", y=" + y + ", width=" + width + ", height=" + height + "]";
	}
...
/*
 * Klassen endrer størrelsen på et figur-objekt. Endringen blir brutt ned i
 * mange små steg, slik at man oppnår en animasjonseffekt.
 */
...
class EGRunner implements Runnable {
	...
	/*
	 * Ufører brukerprogrammet og avslutter med å legge de siste, ventende stegene
	 * inn på køen.
	 */
	public void run() {
		app.run();
...
/*
 * Klassen brukes for å gjøre et geometriobjekt synlig/usynlig underveis i en
 * animasjon.
 */
...
/*
 * Abstrakt superklasse for alle oppgaver (steg). Hovedtråden legger slike steg
 * inn på en kø, som event-tråden plukker fra, og utfører etter tur.
 *
 */
abstract class EGStep {
	...
	/*
	 * Svært korte/små forflytninger/reskaleringer på ulike objekt blir utført i
	 * parallell. Denne metoden returnerer true for slike steg.
	 */
	...
	/*
	 * Subklasser som er knyttet til et bestemt figur-objekt, f.eks. en flytting,
	 * overstyrer denne metoden.
	 */
	public EGFigure getFig() {
		return null;
	}
...
/*
 * Klassen brukes til å opprette opprette det grafiske vinduet. Dette må
 * alltid være det første steget (kall på makeWindow må stå først i run).
 */
class EGWindowStep extends EGStep {
	...
	public String toString() {
		return "EGWindowStep: title=" + title + ", width=" + width + ", height=" + height;
	}
